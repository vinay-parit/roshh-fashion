...
export default function Hero() {
  ...
  // Apply a spring to the scroll value for ultra-smooth "buttery" easing
  ...
  // Fade out from 1 to 0 as user scrolls from 0 to 800px
  const opacity = useTransform(smoothScrollY, [0, 800], [1, 0]);
  
  // Parallax move up from 0 to -250px
  const y = useTransform(smoothScrollY, [0, 800], [0, -250]);

  // Scale down slightly as it fades, making it look like it's moving "inside"
  const scale = useTransform(smoothScrollY, [0, 800], [1, 0.9]);

  return (
    <section className={`w-full h-[100dvh] sticky top-0 z-0 flex flex-col bg-[#e6e4df] text-[#111111] ${inter.className} overflow-hidden`}>
      <motion.div style={{ opacity, y, scale }} className="w-full h-full relative flex flex-col pt-4 md:pt-6 origin-center">
      ...
      {/* Navbar */}
      <nav className="relative z-10 flex items-center justify-between px-4 md:px-8 lg:px-12 shrink-0">
        {/* Logo Icon */}
        ...
        {/* Center Nav Links - Removed by request */}
      ...
      <main className="relative z-10 flex-1 flex flex-col px-4 md:px-8 lg:px-12 pt-4 md:pt-8 min-h-0">
        ...
        <div className="max-w-[1400px] shrink-0 flex flex-col gap-4 md:gap-6 lg:gap-8">
          <h1 className={`${anton.className} text-[14vw] md:text-[10vw] lg:text-[8rem] xl:text-[9rem] leading-[0.95] tracking-tight uppercase text-[#111111]`}>
            Developer.<br />
            Designer. who<br />
            crafts <span className="relative z-10 inline-block px-2 ml-1">
              <span className="absolute bottom-[2%] -left-4 -right-4 h-[38%] bg-[#fcd34d] -z-10 -rotate-2 origin-bottom-left"></span>
              magic.
            </span>
          </h1>
        ...
        <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8 lg:right-12 w-full max-w-[800px] h-[200px] md:h-[350px] pointer-events-none flex justify-end items-end">
          <div className="relative w-full h-full flex justify-end items-end">
            {/* Back Card */}
            <div className="absolute right-12 md:right-32 bottom-4 md:bottom-12 w-64 md:w-[400px] h-36 md:h-[250px] border-[3px] border-[#111111] rounded-t-lg md:rounded-lg overflow-hidden bg-white shadow-2xl rotate-2 transition-transform hover:rotate-0 hover:z-20 pointer-events-auto">
              ...
              <div className="relative w-full h-[calc(100%-1.25rem)] md:h-[calc(100%-2rem)]">
                <Image src="/hero-final.jpg" alt="Project 1" fill className="object-cover" />
              </div>
            </div>

            {/* Front Card */}
            <div className="absolute right-0 bottom-0 w-64 md:w-[400px] h-36 md:h-[250px] border-[3px] border-[#111111] rounded-t-lg md:rounded-lg overflow-hidden bg-white shadow-2xl -rotate-2 z-10 transition-transform hover:rotate-0 pointer-events-auto">
              <div className="w-full h-5 md:h-8 border-b-[2px] md:border-b-[3px] border-[#111111] flex items-center px-2 md:px-3 bg-gray-50">
                <div className="flex gap-1 md:gap-1.5">
                  <div className="w-1.5 h-1.5 md:w-2.5 md:h-2.5 rounded-full bg-red-400"></div>
                  <div className="w-1.5 h-1.5 md:w-2.5 md:h-2.5 rounded-full bg-amber-400"></div>
              ...
              <div className="relative w-full h-[calc(100%-1.25rem)] md:h-[calc(100%-2rem)]">
                <Image src="/hero-landscape.jpg" alt="Project 2" fill className="object-cover" />
              </div>
            </div>
          </div>
        </div>
